...
export default function KanjiPage() {
  ...
  if (kanjiList.length === 0) return <div className="p-8 text-center text-gray-400">Loading...</div>;
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-50">
        <div className="max-w-4xl mx-auto px-4 h-14 flex items-center gap-4">
          ...
          <span className="text-sm text-gray-400">{progress.size}/{kanjiList.length} hafal</span>
          <div className="ml-auto flex gap-1 bg-gray-100 rounded-xl p-1">
            {(['list', 'flashcard', 'quiz'] as ViewMode[]).map((v) => (
              <button key={v} onClick={() => { setView(v); setCardIndex(0); setFlipped(false); setQuizIndex(0); setQuizScore(0); setShowResult(false); }}
                className={`px-3 py-1 text-xs font-medium rounded-lg transition-all ${view === v ? 'bg-white shadow-sm text-primary' : 'text-gray-500'}`}>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        ...
        {view === 'list' && (
          <>
            <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
              <button onClick={() => setFilterSet(0)} className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap ${filterSet === 0 ? 'bg-primary text-white' : 'bg-white border border-gray-200 text-gray-600'}`}>Semua</button>
              {sets.map((s) => (
                <button key={s} onClick={() => setFilterSet(s)} className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap ${filterSet === s ? 'bg-primary text-white' : 'bg-white border border-gray-200 text-gray-600'}`}>Set {s} ({kanjiList.filter((k) => k.set_number === s).length})</button>
              ))}
            </div>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
              {filtered.map((k) => (
                <div key={k.id} onClick={() => toggleHafal(k.id)}
                  className={`p-3 rounded-xl border-2 text-center cursor-pointer transition-all hover:shadow-md ${progress.has(k.id) ? 'border-green-500 bg-green-50' : 'border-gray-100 bg-white'}`}>
                  <div className="text-2xl mb-1">{k.karakter}</div>
                  <div className="text-xs text-gray-500">{k.arti}</div>
                  <div className="text-[10px] text-gray-400">{k.cara_baca_onyomi || ''} {k.cara_baca_kunyomi || ''}</div>
        ...
        {view === 'flashcard' && current && (
          <div className="max-w-sm mx-auto">
            <div className="text-center text-sm text-gray-400 mb-4">{cardIndex + 1} / {fcList.length}</div>
            <div onClick={() => setFlipped(!flipped)} className="bg-white rounded-2xl border-2 border-gray-100 p-12 text-center cursor-pointer hover:shadow-lg transition-all min-h-[280px] flex flex-col items-center justify-center">
              {!flipped ? (
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500">Kunyomi: {current.cara_baca_kunyomi || '-'}</div>
                  <div className="text-sm text-gray-500">Onyomi: {current.cara_baca_onyomi || '-'}</div>
                  <div className="text-xs text-gray-400 mt-2">Level: {current.level} | Set {current.set_number}</div>
                </div>
              )}
            </div>
            <p className="text-center text-xs text-gray-400 mt-2">Tap kartu buat balik</p>
        ...
        {view === 'quiz' && (
          <div className="max-w-md mx-auto">
            {showResult ? (
              ...
              <div>
                <div className="text-center text-sm text-gray-400 mb-4">Soal {quizIndex + 1} / {quizList.length}</div>
                <div className="bg-white rounded-2xl p-8 text-center border border-gray-100 mb-6">
                  <div className="text-6xl mb-2">{quizMode === 'baca' ? qCurrent.karakter : qCurrent.arti}</div>
                  <p className="text-gray-500 text-sm">{quizMode === 'baca' ? 'Pilih arti yang benar' : 'Pilih kanji yang benar'}</p>
                </div>
